...
MembershipTypes.helpers({
	...
	ownerSchema: function(role){
		if(!this.multipleOwnerRoles()){
			role = this.ownerRole().role;
		}
		console.log("role " + role)
		return new SimpleSchema(generateOwnerSchema(this._id, role));
	},
	membershipSchemaObj: function(){
		return generateSchema(this._id);
	},
	...
	ownerRoles: function(){
		return _.where(this.roles(), {canBeOwner: true});
	},
	multipleOwnerRoles: function(){
		return (this.ownerRoles().length > 1);
	},
	ownerRole: function(){
		return this.ownerRoles()[0];
	}
});
...
function generateOwnerSchema(membershipTypeId, role){
	var membershipType = MembershipTypes.findOne({_id: membershipTypeId});
	var ownerSchema = _.findWhere(membershipType.membershipPeople, {role: role});
	ownerSchema.role = {
		type: String,
		value: "role",
		defaultValue: role
	}
	console.log(ownerSchema);
	return ownerSchema;
}
